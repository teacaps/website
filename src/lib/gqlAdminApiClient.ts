declare const Oxygen: { env: Record<string, string> };
...
export async function adminApiClient(query: string, variables: Record<string, unknown> = {}) {
	...
	const json: { errors?: Array<{ message: string }>; data: unknown } = await response.json();
